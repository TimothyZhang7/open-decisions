"""Optional hosted Jev baseline for the Tetris comparison; synthetic state only."""
import argparse
...
from jev_client import ROOT, TypeSafeClient, TypeSafeError, normalized_probabilities
...
def read_choice(body, question, options):
    try:
        ...
        raise TypeSafeError("TypeSafe returned an invalid decision; gravity continues without applying it.") from None
...
def decide_action(client, snapshot):
    ...
    if planned:
        ...
        target = {**graph['options'][plan_answer['choice']], 'piece_id': snapshot['active']['id'],
                  'board_key': board_key(snapshot['board']), 'selected_by': 'jev',
...
def create_server(client, worker, host="127.0.0.1", port=8767):
        ...
        def do_GET(self):
            ...
            if path == "/api/info":
                return self.send(200, {"model": client.model, "shapes": SHAPES,
                                       ...
                                       "control_instructions": CONTROL_INSTRUCTIONS, "goal_lines": 10, "backend": "typesafe"})
            if path == "/health":
...
def main():
    ...
    try:
        client = TypeSafeClient(args.model)
    except ValueError as error:
        parser.error(str(error))
    worker = ThreadPoolExecutor(max_workers=1, thread_name_prefix="jev-tetris")
    server = create_server(client, worker, args.host, args.port)
    print(f"Jev Tetris ready at http://{args.host}:{server.server_port}", flush=True)
    try:
